...
def get_baseline_preds():
    ...
    df['prompt'] = df['informal_prompt'].tolist()
    df['prompt'] = df['prompt'].apply(lambda x: INSTRUCT_PRE + INSTRUCT_PATTERN[:-4].format(x.split('''User's argument:''')[0], x.split('''User's argument:''')[1]))
